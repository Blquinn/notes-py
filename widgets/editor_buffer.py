...
class UndoableInsert:
    ...
    def __init__(self, text_iter: Gtk.TextIter, text: str, length: int):
...
class UndoableBuffer(Gtk.TextBuffer):
    ...
    def __init__(self):
        ...
        """
        super(UndoableBuffer, self).__init__()
        # Do we really need to limit the size of the undo/redo bufs?
        self.undo_stack: Deque[Union[UndoableInsert,  UndoableInsert]] = deque(maxlen=2_000)
        self.redo_stack: Deque[Union[UndoableInsert, UndoableDelete]] = deque(maxlen=2_000)
    ...
    def _on_insert_text(self, textbuffer: Gtk.TextBuffer, text_iter: Gtk.TextIter, text: str, length: int):
    ...
    def _on_delete_range(self, text_buffer: Gtk.TextBuffer, start_iter: Gtk.TextIter, end_iter: Gtk.TextIter):
        def can_be_merged(prev, cur):
        ...
        if can_be_merged(prev_delete, undo_action):
            if prev_delete.start == undo_action.start:  # delete key used
                ...
                prev_delete.text = f'{undo_action.text}{prev_delete.text}'
    ...
    def _begin_not_undoable_action(self):
        """don't record the next actions

        toggles self.not_undoable_action"""
        self.not_undoable_action = True

    def _end_not_undoable_action(self):
        """record next actions
    ...
    def undo(self):
        ...
        if not self.undo_stack:
            return
        self._begin_not_undoable_action()
